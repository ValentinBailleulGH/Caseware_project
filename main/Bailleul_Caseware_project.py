...
def code_extractor(FICHIER_DE_BASE, LISTE_DES_CODES_COMPLETE):
    ...
    Args:
        FICHIER_DE_BASE: the file containing all existing codes
        LISTE_DES_CODES_COMPLETE: the file containing all potential keys
            that can be seen on the left-hand side of the "=" sign in FICHIER_DE_BASE
    Returns:
...
# Every printing function

def main_print(message, title):
    """
...
def print_opening_error():
    ...
    """
    try:
        main_print(ERROR_DICT.get("Opening error"), "Opening error")
    except:
        # Mac, Linux ...
...
def print_writing_is_done(counter):
    ...
    N = str(counter)
    try:
        main_print(N+ERROR_DICT.get("Successfully copied"), "Successfully copied")
    except:
        # Mac, Linux ...
        print("Done !\n\n"+N+" elements have been added to fichier_de_base.txt")
